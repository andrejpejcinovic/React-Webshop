...
const slice = createSlice({
  ...
  reducers: {
    addToBasket: (state, action) => {
      const { items } = state;
      state.items =
        (hasItem(items, action.payload.id) &&
          setAmount(items, action.payload.id, 1)) ||
        combine(items, action.payload);
      save(name, state.items);
    },
    removeFromBasket: (state, action) => {
      state.items = getBut(state.items, action.payload);
      save(name, state.items);
    },
    increaseQuantity: (state, action) => {
      state.items = setAmount(state.items, action.payload, 1);
      save(name, state.items);
    },
...
export const getTotal = (items) =>
  items.reduce((acc, el) => acc + el.qnty * el.price, 0).toFixed(2);
